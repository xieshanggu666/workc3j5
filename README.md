...
- **卡牌成长树**：路线上的锻造节点花金币为指定卡牌实例点亮成长节点——三系主干
  （锋锐/强效/精炼）各自延伸出带前置条件的深层节点与终式，锋锐系第二层为
  **互斥分支**（开刃 ⇄ 坚壁，选其一即永久锁定另一路及其终式）；成长选择与累计
  成本按卡牌实例保存（`growth`/`growth_cost`），同名卡为独立实例、互不影响；
  成长即时换算生效卡牌（伤害/格挡/抽牌/费用/首击加成等），联动战斗结算、
  商店移除（已投入成本随实例一并删除并在交易记录留痕）与跨章继承（随交接快照
  带入下一章）；旧版扁平锻造记录（`forges`）自动迁移为等价成长节点，续局与
  回放遵循同一套成长规则（旧规则录制的日志检出规则漂移 mismatch，但不报错）
...
锻造同名卡独立成长、**成长树前置链与互斥分支（缺前置/互斥已占/重复点亮 400、节点价扣款与累计成本）、
首击成长只作用于回合第一张攻击牌、跨真实锻造节点（多章远征）点亮整条链的整程回放逐位一致、
旧版扁平 forges 迁移为等价节点且数值等价、2.2.0 旧规则 forge 日志检出 mismatch 不报错**、
锻造贯通战斗/奖励/续局/回放、旧档迁移、
商店库存确定性/续局一致、购买卡牌/遗物（扣款/售罄/失败回退）、移除指定实例（递增价/牌组下限/**沉没成长成本留痕**）、
...
锻造行动（成长树）：`{action:"forge", card:<卡牌实例 uid>, node:<成长节点 id>}`
（旧客户端仍可传 `branch:"sharpen"|"empower"|"refine"`，等价于点亮对应 Tier-1 主干）。
- 节点价格随定义返回（Tier-1 25 / Tier-2 50 / 终式 75，视口 `forge_cost` 为基准价、
  `forge_tree.nodes[].cost` 为逐节点价）；视口的 `growth_options[uid]` 给出每张实例
  当前可点亮的节点（前置已满足、互斥未占、尚未点亮），`deck[].growth/growth_cost`
  携带已点亮节点与累计投入。
- 校验：实例不存在 / 节点未知或为迁移专用节点 / 前置未满足 / 互斥已占 / 重复点亮 /
  金币不足一律 400（校验先于扣款，零副作用）；同一锻造节点仅可成长一次，
  重复请求返回 409 且不扣款。
- 成长树结构（Tier-2 中开刃/坚壁同属互斥组 `edge_or_bulwark`）：
  - 锋锐 sharpen →（开刃 edge ⇄ 坚壁 bulwark）→ 歼灭 annihilator / 金城 fortify
  - 强效 empower → 过载 overload / 共鸣 resonance → 超凡 transcendent / 乘势 momentum
  - 精炼 refine → 流转 flux / 灵感 inspiration → 俭用 frugality / 洞见 insight
  - 共鸣/乘势仅强化「本回合打出的第一张攻击牌」（引擎按回合攻击计数，续局/回放一致）；
    俭用对 0 费牌改为打出时额外 +1 能量。
- 静态定义：`GET /api/forge-tree` 返回节点与互斥组（与视口 `forge_tree` 同构）。
- 旧档迁移：实例的扁平 `forges:[分支…]` 在首次载入时逐条映射为等价成长节点
  （每系前 3 次映射到 Tier-1/Tier-2/Tier-3，第 4 次起映射到 `~` 开头的迁移专用节点，
  逐值复刻旧规则的无限叠加、不占互斥位），并按节点价回填 `growth_cost`；
  战斗内实例副本一并迁移。迁移步事件带 `migrated:true`，回放按 legacy 处理本步校验点。
...
- 卡牌实例：`run.card_instances` 为 `uid -> {id, growth:[成长节点...], growth_cost}`，
  牌组/牌堆/手牌只存 uid；引擎在打牌时用 `forging.effective_card(base, growth)`
  即时换算生效卡牌（费用/数值/首击/能量），因此成长/商店购卡/商店移除自动贯通
  战斗结算、续局、跨章继承与回放（初始牌组 7 张，商店可增减）。
- 成长树：节点数据驱动（`forging.FORGE_NODES`，含 `requires`/`mutex_group`/`cost`），
  `validate_unlock` 统一校验前置与互斥，`available_nodes` 为在线/回放共用的纯派生；
  `~` 前缀节点只在旧档迁移时产生、锻造台不可选。
