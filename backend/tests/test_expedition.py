...
def test_chapter_clear_records_carry_and_advance(client):
    ...
    rec["state"]["card_instances"][uid]["growth"] = ["sharpen"]
    rec["state"]["card_instances"][uid]["growth_cost"] = 25
    rec["state"]["health"] = 40
    ...
    assert forged["growth"] == ["sharpen"] and forged["growth_cost"] == 25
    ...
    assert carried["growth"] == ["sharpen"] and carried["growth_cost"] == 25
    # 新章为新图（章节种子派生）
